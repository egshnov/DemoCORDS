...
namespace algos {
    ...
    private:
        ...
        long double p = 1e-3; // max allowed probability of a false positive result for the chi - squared test
        ...
        unsigned long long d_max = 70; //max amount of categories for the chi-squared test in case the data is not skewed
        ...
        bool TooMuchStructuralZeros(const std::vector<std::vector<size_t>> &n_i_j, size_t col_i, size_t col_k);

        long double ComputeChiSquared(const std::vector<std::vector<size_t>> &n_i_j, const std::vector<size_t> &n_i,
